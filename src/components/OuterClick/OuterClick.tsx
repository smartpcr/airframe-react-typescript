...
export interface IOutClickProps {
...
*/
class OuterClick extends React.Component<IOutClickProps> {
    ...
    handleDocumentClick = (evt: Event) => {
        if(this.props.active) {
            ...
            const isExcluded = _.some(this.props.excludedElements,
                // eslint-disable-next-line react/no-find-dom-node
                (element: Element) => evt.target && element && ReactDOM.findDOMNode(element)?.contains(evt.target as Node));

            if (!isExcluded && !domElement?.contains(evt.target as Node)) {
